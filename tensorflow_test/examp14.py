...
def get_batch():
    ...
    #plt.show()
    return [seq[:,:,np.newaxis],res[:,:,np.newaxis],xs]    #return seq,res--->shape(batch,step,input)
...
class LSTMRNN(object):
    ...
    def compute_cost(self):
        losses=tf.nn.seq2seq.sequence_loss_by_example([tf.reshape(self.pred,[-1],name='reshape_pred')],[tf.reshape(self.ys,[-1],name='reshape_target')],[tf.ones([self.batch_size*self.n_steps],dtype=tf.float32)],average_across_timesteps=True,softmax_loss_function=self.msr_error,name='losses')
